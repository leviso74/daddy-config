...
export async function initializeSecretRotation(): Promise<void> {
  ...
  // Register hooks for secrets that need runtime updates
  sm.registerRotationHook({
    secretId: 'FX_API_KEY',
    onRotate: (newValue: string) => {
      console.log('[secrets] FX_API_KEY rotated');
    },
  });
  ...
  sm.registerRotationHook({
    ...
    onRotate: (newValue: string) => {
      console.log('[secrets] Primary webhook secret rotated');
    },
  });
}