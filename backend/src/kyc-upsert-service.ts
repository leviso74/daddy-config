...
export class KycUpsertService {
  ...
  async upsert(record: KycRecord): Promise<void> {
    if (!VALID_STATUSES.includes(record.kyc_status)) {
      throw new ValidationError(
        `Invalid kyc_status "${record.kyc_status}". Must be one of: ${VALID_STATUSES.join(', ')}`
      );
    }
  ...
  async getStatusForUser(userId: string): Promise<UserKycStatus> {
    const result = await this.pool.query<{
      ...
      `SELECT anchor_id, kyc_status, kyc_level, verified_at, expires_at, rejection_reason
    ...
    const anchors: AnchorKycRecord[] = rows.map(r => ({
      anchor_id: r.anchor_id,
      kyc_status: r.kyc_status,
      kyc_level: (r.kyc_level as any) ?? undefined,
    ...
    const hasApproved = rows.some(
      r => r.kyc_status === 'approved' && (!r.expires_at || r.expires_at > now)
    );
