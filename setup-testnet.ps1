# Daddy-config Testnet Setup Script (PowerShell)
...
Write-Host "🚀 Daddy-config Testnet Setup Script" -ForegroundColor Cyan
...
Write-Host "Step 4: Building and deploying Daddy-config contract..." -ForegroundColor Yellow

# Check if we're in the right directory
if (-not (Test-Path "Cargo.toml")) {
    Write-Error "Cargo.toml not found. Please run this script from the Daddy-config root directory."
    exit 1
}
...
if (-not (Test-Path "target/wasm32-unknown-unknown/release/daddy-config.wasm")) {
...
# Deploy contract
Write-Info "Deploying Daddy-config contract..."
...
$EnvContent = @"
# Daddy-config Testnet Configuration
...
# Contract IDs
DADDY-CONFIG_CONTRACT_ID=$ContractId
...
$TestEnvContent = @"
# Daddy-config Testnet Integration Test Configuration
# Generated by setup-testnet.ps1 on $(Get-Date)
...
Write-Host "==================================" -ForegroundColor Gray
Write-Host ""
Write-Host "Your Daddy-config testnet environment is ready:" -ForegroundColor Gray
Write-Host ""
Write-Host "Contract Information:" -ForegroundColor Blue
Write-Host "  Daddy-config Contract: $ContractId"
